 ...
 *
 * API 키는 앱을 연 뒤 화면 오른쪽 아래 ✨ 버튼 → [API 키 설정] 에서
 * 넣어주세요. 키는 그 기기의 브라우저에만 저장됩니다.
 * (자세한 방법은 README.md 의 "AI 도우미 사용하기" 를 읽어주세요)
 * ---------------------------------------------------------------- */
const AI_CONFIG = {
  /** Gemini API 주소 (보통 바꿀 일이 없습니다) */
   ...
   *
   * ※ 구글이 모델 이름을 자주 바꾸기 때문에, 아래 이름이 맞지 않을 수 있습니다.
   *   앱의 [AI 설정] 화면에서 [사용 가능한 모델 불러오기] 를 누르면
   *   내 API 키로 실제 쓸 수 있는 모델 목록을 확인하고 바로 고를 수 있습니다.

...
### 앱이 뜨는 속도에 대해 (2026-08 개선)

처음 켤 때 기다리는 시간을 줄이기 위해 아래를 바꿨습니다.

| 무엇을 | 어떻게 |
| --- | --- |
| `data/transit.js` (약 270KB) | 시작할 때 받지 않고, **[현지] 탭에 들어갈 때** 받습니다 |
| 로그인 화면 그림 (약 220KB) | **로그인 화면을 실제로 보여줄 때만** 받습니다 (이미 로그인했다면 안 받습니다) |
| 서버 왕복 | 예전에는 `세션 확인` + `자료 받기` 두 번이었는데, **한 번**으로 줄였습니다 |
| 저장해 둔 자료 | 서버 응답을 기다리지 않고 **먼저 화면에 띄우고**, 최신 자료는 뒤에서 받아 바꿔 끼웁니다 |
| 서비스 워커 | 앱 파일을 "네트워크 먼저" → **"저장된 것 먼저"** 로 바꿔, 켤 때마다 다시 받지 않습니다 |
| 아이콘 폰트 | 화면 그리기를 막지 않도록 나중에 적용합니다 |
| Apps Script | 시트·설정을 요청 한 번 안에서 **한 번만 읽도록** 기억해 둡니다 |

그 결과 로그인해 둔 상태에서 앱을 다시 켤 때, 서버 응답을 기다리지 않고
**바로 오늘 화면이 보입니다.** (동기화 막대에 "최신 정보 확인 중…" 이라고 표시됩니다)

### 첨부 PDF 를 앱 안에서 보는 방법 (2026-08 개선)

Drive 의 미리보기 화면을 작은 창(iframe)에 넣으면 Drive 자체 보안 규칙에 막혀
검게 보이는 일이 많습니다. 그래서 이 앱은 **파일 내용만 받아와** 브라우저에
내장된 PDF 뷰어로 엽니다. 이번에 아래를 고쳤습니다.

* Apps Script 가 조각마다 **파일 전체를 통째로 읽던 것**을 고쳐, 필요한 구간만
  Range 요청으로 받아옵니다. (6MB 문서 기준 읽는 양이 1/3 로 줄었습니다)
* 파일 정보와 권한 확인 결과를 **6시간 동안 기억**해, 조각마다 시트를 다시 읽지 않습니다.
* 조각을 한 줄로 기다리지 않고 **동시에 3개씩** 받아옵니다.
* PDF 에도 **`… 42%` 진행률**을 보여줍니다.
* 같은 파일을 두 번 눌러도 **한 번만** 내려받습니다.
* 아이폰에서 `전체 화면으로 보기` 가 팝업 차단에 막히지 않도록 여는 방식을 바꿨습니다.
...
# AI 도우미 (키는 스크립트 속성 GROQ_API_KEY 에만 있고 응답에 포함되지 않습니다)
aiStatus    - 서버에 키가 준비됐는지 + 서버가 정한 모델 순서
aiChat      - 대화 내용을 받아 Groq 을 대신 호출하고 답변만 돌려줍니다
aiModels    - 이 키로 쓸 수 있는 모델 이름 목록 (30분 캐시)
              ※ 한 번에 최대 3개 모델까지만 시도하고,
                직전에 성공한 모델을 먼저, 실패한 모델은 10분간 뒤로 미룹니다
...
**요청은 짧으면 GET, 길면 POST 로 보냅니다.**

```
GET  .../exec?action=saveSchedule&payload={"action":"saveSchedule","token":"...","date":"2026-08-05",...}
POST .../exec        (본문에 같은 JSON — 사진 · 영상 업로드처럼 긴 요청)
```

Apps Script 는 답을 주기 전에 `googleusercontent.com` 으로 한 번 넘기는데,
**POST 는 이때 CORS 헤더가 사라져 브라우저가 막는 일이 있습니다.**
특히 **아이폰 사파리와 안드로이드 크롬**에서 자주 나타납니다.
GET 은 이 과정에서 문제가 없으므로, 주소에 담을 수 있는 짧은 요청은 GET 으로 보냅니다.
(주소가 6000자를 넘으면 자동으로 POST 로 바뀝니다)
...
| `GROQ_API_KEY` | `gsk_...` | **(선택)** AI 도우미용 키. 넣지 않아도 나머지 기능은 모두 동작합니다. 자세한 내용은 [5-1](#5-1-ai-도우미-사용하기) |
| `GROQ_MODELS` | `openai/gpt-oss-120b,openai/gpt-oss-20b` | **(선택)** AI 모델을 쓸 순서. 비워두면 기본값을 사용합니다 |
...
<details open>
<summary><b>시각을 정확히 입력했는데 새로고침하면 몇 분씩 달라짐</b> (예: 12:30 → 12:32) ★중요★</summary>

일정을 "12:30 출발" 로 저장했고, 저장 직후에는 12:30 이 맞게 보이는데,
**나중에 새로고침(⟳)을 하면 12:32 처럼 몇 분 달라져 보이는** 경우입니다.

**원인** : 위 항목과 근본 원인은 같지만(시트가 값을 날짜/시각으로 자동 변환),
드러나는 시점이 다릅니다. 예전에 `repairDateTimeColumns` 를 한 번 실행해서
그때까지 있던 값들은 보호되었더라도, **그 이후에 새로 추가한 일정**은
보호 범위 밖에 있어서 다시 자동 변환될 수 있었습니다. 저장 직후 화면에는
방금 입력한 값이 그대로 보이지만(서버에 다시 묻지 않으므로), 새로고침해서
시트에 저장된 실제 값을 다시 읽어오면 자동 변환된(약간 어긋난) 값이 나타납니다.

**해결 방법**

1. 최신 `Code.gs` 를 Apps Script 에 붙여 넣고 저장합니다.
   이제는 값을 쓰기 **바로 직전**에 그 줄(행)의 날짜·시각 칸을 다시 한 번
   "일반 텍스트" 서식으로 강제합니다. 시트가 예전부터 있었든, 최근에
   만들었든, 몇 번째로 추가되는 행이든 상관없이 저장할 때마다 항상 보호됩니다.
2. **`배포` → `배포 관리` → ✏️ → 버전 `새 버전` → `배포`** 로 다시 배포합니다.
3. 이미 어긋난 값이 있다면 **`repairDateTimeColumns`** 를 한 번 더 실행해 두세요.

배포 후에 새로 저장하는 일정부터는 이런 드리프트가 생기지 않습니다.
</details>

<details>
<summary><b>GitHub Pages 에서 예전 코드가 보이는 캐시 문제</b> ★자주 발생★</summary>

> **v15 부터 바뀐 점** — 앱 파일(HTML/CSS/JS)을 **저장된 것 먼저** 보여주고
> 새 파일은 뒤에서 조용히 확인합니다. 그래서 앱이 훨씬 빨리 뜨는 대신,
> 코드를 새로 올린 뒤 **처음 한 번은 예전 화면이 보일 수 있습니다.**
> 새 버전을 찾으면 앱이 *"새 버전이 준비되었습니다"* 라고 알려주니,
> 그때 한 번 새로고침하면 바로 적용됩니다.
...
**Groq**(그록) 을 사용하며, 앱에 저장된 여행 정보(일정 · 항공 · 숙소 · 예약 · 경로 · 기록 · 경비)를
함께 보내주기 때문에 "**오늘 몇 시에 나가야 해?**" 같은 질문에도 우리 여행 기준으로 답합니다.
앱에 없는 내용(예: "삿포로 8월 날씨 어때?")은 모델이 아는 지식으로 답하고,
확실하지 않으면 **"확실하지 않다"고 밝히도록** 했습니다.

> **2026년 8월, Gemini 에서 Groq 으로 바꿨습니다.**
> Gemini 무료 키가 프로젝트에 따라 사용량이 `0` 으로 잡혀 아예 동작하지 않는 일이 잦았습니다.
> Groq 은 **카드 등록 없이** 무료로 시작할 수 있고, 응답이 아주 빠릅니다(LPU 라는 전용 칩을 씁니다).
> 예전 `GEMINI_API_KEY` 는 그냥 두어도 되고, 지워도 됩니다. 앱은 이제 `GROQ_API_KEY` 만 봅니다.
...
```
[내 브라우저] --(로그인 토큰만)--> [Apps Script] --(API 키)--> [Groq]
                                     ↑
                            키는 여기에만 있습니다
```
...
#### 1) API 키 만들기 (카드 등록 없이 무료입니다)

1. [https://console.groq.com/keys](https://console.groq.com/keys) 에 접속합니다.
2. Google 계정이나 GitHub 계정으로 **로그인(Sign up)** 합니다. *결제 카드는 묻지 않습니다.*
3. **`Create API Key`** 를 누르고, 이름은 아무거나(예: `bestone`) 적습니다.
4. `gsk_` 로 시작하는 긴 문자열이 나옵니다. **복사** 합니다.
   ⚠️ 이 화면을 닫으면 **다시 볼 수 없습니다.** 지금 복사해서 안전한 곳에 붙여 두세요.
...
> **키를 바꾸고 싶을 때**
> [console.groq.com/keys](https://console.groq.com/keys) 에서 옛 키를 지우고 새로 만든 뒤,
> 스크립트 속성 `GROQ_API_KEY` 의 값만 바꾸고 다시 배포하면 됩니다.

### 어떤 모델을 쓰나요 (그리고 왜)

기본 순서는 이렇습니다. **무료 한도가 넉넉하면서 품질이 좋은 순서**로 골랐습니다.

| 순서 | 모델 | 고른 이유 |
| --- | --- | --- |
| 1 | **`openai/gpt-oss-120b`** | Groq 무료 등급에서 **하루 토큰 한도가 가장 넉넉하고**(분당 30회 · 하루 20만 토큰), 답 품질도 가장 좋습니다. 문맥 13만 토큰 |
| 2 | `openai/gpt-oss-20b` | 더 가볍고 빨라서, 1번이 한도에 걸렸을 때 그대로 이어받습니다 |
| 3 | `qwen/qwen3.6-27b` | 위 둘이 모두 막혔을 때를 위한 예비 |

**중요 : 한도는 모델마다 따로 계산됩니다.**
그래서 목록에 여러 개를 적어두면, 1번이 하루 한도를 다 써도 앱이 **자동으로 2번으로 넘어가**
계속 쓸 수 있습니다. 하루에 대략 **질문 100~150번** 정도는 넉넉히 됩니다.

> 참고 : `llama-3.1-8b-instant` 와 `llama-3.3-70b-versatile` 는 2026년 6월에
> Groq 에서 내려갔습니다(deprecated). 그 자리를 대신하는 것이 위의 `gpt-oss` 두 개입니다.

### 웹 검색 도구를 기본으로 끈 이유 ★중요★

Groq 의 `browser_search` 는 **서버가 실제로 웹사이트를 하나씩 돌아다니며 읽는** 방식입니다.
정확하지만 아주 느립니다. 그런데 이 앱은 `브라우저 → Apps Script → Groq` 구조라서,
그 시간 동안 **Apps Script 실행이 먼저 끊깁니다.** 그러면 응답이 저장되지 않아
브라우저가 `script.googleusercontent.com/macros/echo` 에서 **404** 를 받고,
앱에는 `HTTP 404` 만 뜨면서 답이 아예 나오지 않습니다.

> 실제 증상 : 첫 질문("내일 일정 정리해줘" — 앱 자료만으로 답함)은 잘 되는데,
> 그 다음 "맛집 **검색**해줘" 부터 답이 비거나 `HTTP 404` 가 뜹니다.

그래서 `config.js` 의 `AI_CONFIG.USE_SEARCH` 를 **`false`** 로 두었습니다.
꼭 필요하면 `true` 로 바꿀 수 있지만, 답이 늦거나 실패할 수 있습니다.

함께 넣은 안전장치는 이렇습니다.

* Apps Script 가 **45초 예산**을 넘기면 남은 모델을 건너뛰고 지금까지 결과라도 돌려줍니다.
* 사용량 초과(429) 때 `Utilities.sleep` 으로 기다리던 것을 없앴습니다. (실행 시간을 잡아먹던 원인)
* 한 번에 시도하는 모델 수를 3개 → **2개**로 줄였습니다.
* 그래도 `HTTP 404` 가 나면, 앱이 **보낼 내용을 확 줄여 한 번 자동으로 다시 시도**합니다.
...
| 빨간 칸이 끝까지 | 마지막 순위 모델까지 내려옴 (오늘 한도가 거의 찼을 수 있음) |
| 전부 비어 있고 `대기 중` | 아직 한 번도 답하지 않음 (또는 키 없음) |

막대 옆에는 지금 답하고 있는 **모델 이름**이 짧게 표시됩니다. (예: `gpt-oss-120b`)
막대 위에 마우스를 올리면 `지금 모델: … (2번째 / 전체 3개)` 처럼 자세히 나옵니다.
...
| 2 | 스크립트 속성 `GROQ_MODELS` | 쉼표로 구분해 적기 — 두 사람 모두에게 적용됩니다 |
...
```javascript
MODELS: [
  'openai/gpt-oss-120b',   // 무료 한도가 가장 넉넉 + 품질 최상
  'openai/gpt-oss-20b',    // 더 가볍고 빠름 (위가 막히면 이어받음)
  'qwen/qwen3.6-27b'       // 예비
]
```

앞의 모델이 **없거나(404) 사용량이 꽉 차면(429)** 자동으로 다음 모델로 넘어갑니다.
...
`GROQ_MODELS` 에 `openai/gpt-oss-120b,openai/gpt-oss-20b` 처럼 쉼표로 적고
다시 배포하세요.

### 무료로 얼마나 쓸 수 있나요

Groq 무료 등급(Free)의 대략적인 한도입니다. *(2026년 8월 기준, Groq 이 바꿀 수 있습니다)*

| 항목 | 한도 |
| --- | --- |
| 분당 요청 수 | 30회 |
| `openai/gpt-oss-120b` 하루 토큰 | 약 20만 토큰 (질문 100회 안팎) |
| 계정 전체 하루 요청 수 | 최대 14,400회 |
| 요금 | **0원** (카드 등록 없음, 선불 크레딧 없음) |

두 사람이 여행하면서 쓰기에는 넉넉합니다.
더 필요하면 [console.groq.com/settings/billing](https://console.groq.com/settings/billing) 에서
카드를 등록하면 한도가 약 10배가 됩니다. **최소 결제 금액은 없고**, 쓴 만큼만 냅니다.
...
| `GROQ_API_KEY 가 올바르지 않습니다` | 키를 복사할 때 앞뒤 공백이나 줄바꿈이 섞였을 수 있습니다. 스크립트 속성의 값을 지우고 다시 붙여 넣은 뒤 재배포하세요. `gsk_` 로 시작하는지도 확인하세요 |
| `모든 모델을 시도했지만 실패했습니다` | 모델 이름이 바뀌었을 가능성이 큽니다. 위 **모델 순서를 바꾸고 싶을 때** 를 따라 하세요 |
| **`오늘 쓸 수 있는 양을 다 썼습니다`** | 그 모델의 하루 한도를 다 쓴 것입니다. 앱이 자동으로 다음 모델로 넘어가지만, 목록의 모델을 전부 다 썼다면 **내일**이 되어야 합니다. 급하면 ⚙ → `사용 가능한 모델 불러오기` 로 다른 모델을 추가하세요 |
| `사용량이 잠시 가득 찼습니다` | 분당 한도에 걸린 것입니다. 앱이 잠깐 기다렸다 다시 시도하고, 안 되면 다음 모델로 넘어갑니다. 잠시 뒤 다시 물어보세요 |
| `이제 쓸 수 없는 모델입니다` | Groq 이 그 모델을 내렸습니다. 앱이 기억해 두고 6시간 동안 다시 두드리지 않습니다. 자동으로 다른 모델을 씁니다 |
| **`HTTP 404` / `서버가 제때 답을 돌려주지 못했습니다`** | Groq 오류가 아니라 **Apps Script 실행이 너무 오래 걸려 응답이 사라진 것**입니다. (개발자 도구에 `macros/echo` 404 로 보입니다) 앱이 자동으로 한 번 더 줄여서 시도하고, 그래도 안 되면 질문을 짧게 바꾸거나 ⚙ [설정] 에서 `openai/gpt-oss-20b` 를 맨 위로 올려보세요 |
| 설정 화면에 **`gemini-…` 모델이 그대로 남아 있음** | 모델 순서는 브라우저에 저장됩니다. 새 `script.js` 가 적용되면 **자동으로 한 번 정리**됩니다. 그래도 남아 있으면 ⚙ [설정] → `기본값으로` 를 누르세요 |
| `알 수 없는 요청입니다: aiChat` | Apps Script 가 예전 버전입니다. 새 `Code.gs` 를 붙여넣고 다시 배포해 주세요 |
| 답변 말풍선이 **비어 있음** | 질문이 너무 길거나 여행 자료가 길어 분당 토큰 한도에 걸렸을 수 있습니다. 잠시 뒤 짧게 다시 물어보세요 |
| 답이 옛날 일정 기준으로 나옴 | 오른쪽 위 `⟳` 로 자료를 새로 받은 뒤 다시 물어보세요 |
| 오프라인에서 답이 안 나옴 | AI 는 인터넷이 있어야 동작합니다. (앱의 다른 기능은 오프라인에서도 열람 가능) |
| **휴대폰에서만 로그인 · 저장이 안 됨** | 예전 버전은 모든 요청을 POST 로 보냈는데, Apps Script 의 리다이렉트 뒤에 CORS 헤더가 사라져 모바일 브라우저가 막는 일이 있었습니다. 지금은 짧은 요청을 GET 으로 보냅니다. **새 `Code.gs` 로 다시 배포**해야 적용됩니다 |
| 아이폰에서 PDF 가 빈 화면 | 아이폰 사파리는 작은 창(iframe) 안에 PDF 를 못 그립니다. 팝업 아래 `전체 화면으로 보기` 를 눌러주세요 |
...
> 대화 내용은 Apps Script 를 거쳐 Groq 서버로 전송됩니다.
...
| 19-2 | **여러 번 새로 저장해도 유지** | 일정 탭 → `12:30` 으로 새 일정을 여러 개 잇달아 추가 → 매번 `⟳` 새로고침 | 몇 번째로 추가하든 `12:30` 그대로 유지 (드리프트 없음) |
...
| 24 | **아이폰 사파리** | 아이폰 사파리로 로그인 → 일정 저장 | 정상 동작. 개발자도구 없이도 되는지가 중요합니다 |
| 24-1 | **안드로이드 크롬** | 안드로이드 크롬으로 로그인 → 일정 저장 | 정상 동작 |
| 24-2 | **아이폰에서 PDF** | 티켓 PDF 를 누름 | 창에 안 보이면 `전체 화면으로 보기` 로 열림 |
...
| 22-1 | **모델 자동 전환** | `AI 도우미 설정` → 1순위에 없는 이름(`없는-모델-이름`)을 넣고 질문 | 오류 없이 다음 모델로 넘어가 답변. 막대가 한 칸 더 채워짐 |
...
* **Groq API 키도 서버(스크립트 속성)에만 있습니다.** 브라우저로 내려오지 않으므로
...
* 여행이 끝나면
  ...
  3. AI 를 더 쓰지 않는다면 스크립트 속성의 `GROQ_API_KEY` 를 지우거나
     console.groq.com/keys 에서 키를 삭제하세요.

...
> **`config.js` 에 넣지 마세요. 코드 파일 어디에도 넣지 마세요.**
> 이 저장소는 GitHub 에 공개되어 있어서, 코드에 키를 적으면 **누구나 그 키를 가져다 쓸 수 있고
> 요금이 나에게 청구**됩니다. 그래서 이 앱은 키를 **앱 화면 안에서** 넣도록 만들었습니다.

**넣는 곳은 앱 화면 안, 딱 한 군데입니다.**

| 단계 | 하는 일 |
| --- | --- |
| 1 | 앱에 로그인합니다 |
| 2 | 화면 **오른쪽 아래 ✨ 버튼**을 누릅니다 |
| 3 | 채팅창 오른쪽 위 **⚙(톱니바퀴) 버튼** → `AI 도우미 설정` 이 열립니다 |
| 4 | **`Gemini API 키`** 칸에 키를 붙여 넣습니다 (`AIza...` 로 시작합니다) |
| 5 | **`키 저장`** 버튼을 누릅니다 |

*(☰ 메뉴 → `AI 도우미 설정 (API 키)` 로도 같은 화면을 열 수 있습니다.
아직 키가 없는 상태에서 ✨ 버튼을 누르면 이 설정 화면이 자동으로 먼저 열립니다.)*

키는 **그 기기의 브라우저(localStorage)에만** 저장됩니다.
GitHub 에도, 우리 Apps Script 서버에도, Google Sheets 에도 올라가지 않습니다.
그래서 **휴대폰과 PC에서 각각 한 번씩** 넣어주어야 합니다.

### API 키 만드는 법 (무료로 시작할 수 있습니다)

1. [https://aistudio.google.com/apikey](https://aistudio.google.com/apikey) 에 접속합니다.
   (설정 화면의 **`AI Studio 에서 키 만들기`** 버튼을 누르면 바로 열립니다)
2. Google 계정으로 로그인합니다.
3. **`Create API key`** 를 누릅니다.
4. `AIza` 로 시작하는 긴 문자열이 나옵니다. **복사** 합니다.
5. 위의 5단계대로 앱에 붙여 넣고 `키 저장` 을 누릅니다.

> **키를 지키는 요령**
> AI Studio 의 키 상세 화면에서 **웹사이트 제한(HTTP 리퍼러)** 을 걸어두면 더 안전합니다.
> 내 GitHub Pages 주소(예: `https://chichiboo123.github.io/*`)만 허용해 두면,
> 혹시 키가 새어 나가도 다른 사이트에서는 쓰지 못합니다.
> 키를 지우고 싶으면 설정 화면에서 칸을 **비우고 `키 저장`** 을 누르면 됩니다.
...
기본 순서는 `config.js` 의 `AI_CONFIG.MODELS` 에 적혀 있습니다.
...
그래도 전부 실패한다면, 구글이 이름을 바꾼 것입니다. **코드를 고치지 않고** 이렇게 해결하세요.

1. `AI 도우미 설정` 화면을 엽니다.
2. **`사용 가능한 모델 불러오기`** 를 누릅니다.
   → 내 API 키로 **실제로 쓸 수 있는** 모델 이름이 알약 모양으로 나옵니다.
3. 쓰고 싶은 이름을 누르면 목록 맨 아래에 추가됩니다.
4. `↑` 버튼으로 순서를 올리고, `✕` 로 안 쓰는 모델을 뺍니다.
5. 이렇게 만든 목록은 브라우저에 저장되어 `config.js` 기본값보다 **먼저** 사용됩니다.
   되돌리고 싶으면 **`기본값으로`** 를 누르세요.

### AI 도우미 관련 오류 해결

| 증상 | 원인과 해결 |
| --- | --- |
| ✨ 버튼을 눌러도 설정 화면만 뜸 | 아직 API 키를 저장하지 않았습니다. 위 5단계를 진행하세요 |
| `API 키가 올바르지 않습니다` | 키를 복사할 때 앞뒤 공백이나 줄바꿈이 섞였을 수 있습니다. 다시 복사해 붙여 넣으세요 |
...
| PC에서는 되는데 휴대폰에서 안 됨 | 키는 기기마다 따로 저장됩니다. 휴대폰에서도 한 번 넣어주세요 |

> **AI 답변을 그대로 믿지 마세요.**
> 열차 시각 · 영업시간 · 요금처럼 틀리면 곤란한 정보는 반드시 공식 사이트에서 다시 확인하세요.
> 대화 내용은 Google 의 Gemini 서버로 전송됩니다. 주민등록번호 · 카드번호 같은
> 민감한 정보는 입력하지 마세요.
...
| 21 | **PDF 앱 안에서 보기** | 첨부한 PDF 미리보기를 누름 | 새 창이 아니라 앱 안 팝업에서 열림. `새 탭에서 열기` 버튼도 있음 |
| 22 | **AI 도우미** | ✨ 버튼 → 키 저장 → "오늘 몇 시에 나가야 해?" | 우리 일정을 근거로 답하고, 배터리 막대에 모델 이름 표시 |
...
| PDF 팝업이 비어 있음 | Drive 파일 공유가 "링크가 있는 사용자"인지 확인하세요. 그래도 안 보이면 팝업 안의 `새 탭에서 열기` 를 누르세요 |

...
| **기록** | 두 사람의 기록을 한 줄기로 모은 피드(작성자 표시), 사진, 지출, 여행 요약 |
...
MODELS: [
  ...
  'gemini-2.5-flash-lite'    // 5순위
]
```

앞의 모델이 **없거나(404) 사용량이 꽉 차면(429 · 503)** 자동으로 다음 모델로 넘어갑니다.

**적어둔 이름이 전부 없는 이름이면, 앱이 스스로 고칩니다.**
실제 쓸 수 있는 모델 목록을 받아와 그중 가장 알맞은 것으로 바꿔 답하고
("… 로 바꿔 답했습니다" 안내가 뜹니다), 다음부터는 그 목록을 씁니다.
...
| `사용량이 많습니다` 가 계속 나옴 | 무료 사용량을 넘었습니다. 잠시 뒤에 다시 시도하거나, AI Studio 에서 결제를 연결하세요 |

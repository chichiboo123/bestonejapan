...
| **현지** | 노선도·길찾기(도쿄·삿포로), 일본어 표현(152개), 단어장(88개), 역·버스 정보, 이동 경로 |
| **기록** | 두 사람의 기록을 한 줄기로 모은 피드(작성자 표시), 사진, 지출, 여행 요약 |

그리고 화면 위쪽 **🔍 버튼**(PC 에서는 `Ctrl + K`)을 누르면
일정 · 예약 · 항공 · 숙소 · 일본어 표현 · 단어 · 역 · 버스 · 경로 · 기록 · 지출을
**한 번에 검색**할 수 있습니다. 결과를 누르면 해당 화면으로 바로 이동합니다.
각 목록(일정 / 예약 / 역 / 버스 / 경로 / 표현 / 단어)에도 검색창이 따로 있습니다.
...
**버스**는 노선·시각표가 자주 바뀌고 지역마다 제각각이라 **자동 길찾기를 넣지 않았습니다.**
대신 **현지 → 버스 정보** 에 도쿄와 삿포로의 **타는 방법 차이**(앞문/뒷문, 선불/후불, 정리권 필요 여부)를
정리해 두고 공식 사이트로 연결했습니다. 실제 노선은 직접 저장해 쓰시면 됩니다.
...
| 9-1 | **전체 검색** | 화면 위 `🔍` → `삿포로` 입력 | 숙소 · 표현 · 역 · 버스 · 경로가 묶음별로 나오고, 누르면 해당 화면으로 이동 |
